...
import busybar_mcp.time as time_module
...
def _mock_client_with_exception(exc_class=ConnectionError):
    ...
    for attr in ["get_time", "get_time_timezone", "get_time_tzlist"]:
...
@pytest.mark.parametrize("tool_func_name,api_method_attr", TIME_TOOLS)
def test_tool_happy_path(tool_func_name, api_method_attr):
    with patch.object(time_module, "_make_api") as mock_make_api:
        mock_client = _mock_client_for_happy_path()
        mock_make_api.return_value = mock_client

        fn = getattr(time_module, tool_func_name)
        result = fn()
